...
export default function Overview({
  ...
  const network = Number(
    (system?.network?.rxSecKB ?? 0) + (system?.network?.txSecKB ?? 0),
  );
  ...
  const graphData = [
    ...
    {
      t: "t-2",
      cpu: Math.max(0, cpu - 0.5),
      memory: Math.max(0, memory - 0.5),
    },
    { t: "now", cpu, memory },
  ];
    ...
    <div className="space-y-3 pt-2">
      <section className="panel">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs font-semibold ${status === "Stable" ? "text-accent" : status === "Warning" ? "text-amber-300" : "text-red-400"}`}
          >
            {status.toUpperCase()}
          </span>
        </div>
        <p className="mt-2 text-sm text-muted">
          Host: {system?.hostname || "loading"} · OS: {system?.os || "loading"}{" "}
      ...
      <section className="panel">
        ...
        <div className="mt-2 w-full min-h-[300px]">
        ...
        <div className="panel">
          ...
          <div className="mt-3 space-y-1 text-sm text-muted">
            <p>
              {status === "Stable"
                ? "No active alerts"
                : "Threshold exceeded. Review host metrics."}
            </p>
            <p>
              Last update:{" "}
              {system?.updatedAt
                ? new Date(system.updatedAt).toLocaleTimeString()
                : "n/a"}
            </p>
